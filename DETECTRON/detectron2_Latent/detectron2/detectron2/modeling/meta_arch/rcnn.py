...
class LatentRCNN(GeneralizedRCNN):
    ...
    def sample_augment(
        ...
        class_labels = []
        each_masks = []
        sample_class_masks = [] # 각 클래스별 마스크 저장을 위한 딕셔너리
        ...
        for aug in augmentations:
            ...
            for _ in range(nsamp):
                ...
                aug_features = {
                    k: self.squash_cat[k](torch.cat([x, sample[None, :, None, None].expand(-1, -1, x.shape[2], x.shape[3])], dim=1))
                ...
                instance = Instances(instances._image_size, pred_boxes=boxes, pred_masks=masks,
                                    pred_classes=instances.pred_classes, scores=instances.scores)

                all_instances.append(instance)
                latent_variables.append(sample.cpu().numpy())  # Store latent variables  
                class_labels.append(instances.pred_classes.cpu().numpy())  # Store class labels
                class_mask_dict = {}
                for cls, mask in zip(instances.pred_classes, masks):
                    if cls.item() not in class_mask_dict:
                        class_mask_dict[cls.item()] = mask.sum()
                    else:
                        class_mask_dict[cls.item()] += mask.sum()

                sample_class_masks.append(class_mask_dict)
        
        # print(sample_class_masks)
        return all_instances, sample_class_masks
    def calculate_confidence(self, masks):
        stacked_masks = torch.stack(masks, dim=0)
        confidence_scores = stacked_masks.float().mean(0)  # 픽셀 위치별 평균 신뢰도 계산
    ...
    def inference(
        ...
        instances,_ = self.sample_augment(batched_inputs[0], self.cfg.AGREEMENT.NSAMPLES, self.cfg.AGREEMENT.AUGMENTATIONS)
        # latent_variables, class_labels = self.sample_augment(batched_inputs[0], self.cfg.AGREEMENT.NSAMPLES, self.cfg.AGREEMENT.AUGMENTATIONS)
    ...
    def uncertainties_inference(
        ...
        Instances, uncertainties_dict = self.sample_augment(batched_inputs[0], self.cfg.AGREEMENT.NSAMPLES, self.cfg.AGREEMENT.AUGMENTATIONS)
        # instances, _ = self.sample_augment(batched_inputs[0], self.cfg.AGREEMENT.NSAMPLES, self.cfg.AGREEMENT.AUGMENTATIONS)
        ...
        ###Uncertainty###
        class_values = {}
        for uncertainty in uncertainties_dict:
            for key, value in uncertainty.items():
                if key in class_values:
                    class_values[key].append(value.item())  # Convert tensor values to regular numbers and store
                else:
                    class_values[key] = [value.item()]

        # Ensure all classes have the correct number of samples
        for cls, values in class_values.items():
            if len(values) < self.cfg.AGREEMENT.NSAMPLES:
                # Append zero to make up for missing samples
                values.extend([0] * (self.cfg.AGREEMENT.NSAMPLES - len(values)))
            print(f"Class {cls}: {values}")

        # Normalize values for each class
        class_scaled_values = {
            cls: [(val - min(values)) / (max(values) - min(values)) if max(values) > min(values) else 0 for val in values]
            for cls, values in class_values.items()
        }
        class_coeff_variation = {}
        for cls, values in class_values.items():
            # Ensure all classes have the correct number of samples
            if len(values) < self.cfg.AGREEMENT.NSAMPLES:
                values.extend([0] * (self.cfg.AGREEMENT.NSAMPLES - len(values)))
            ...
            if mean != 0:  # Avoid division by zero
                ...
                coeff_var = 0  # If mean is 0, set coefficient of variation to 0

            class_coeff_variation[cls] = coeff_var
            # print(f"Class {cls}: Coefficient of Variation = {coeff_var:.3f}")


        return ret, class_coeff_variation
    # def uncertainties_inference(
    #     self,
    #     batched_inputs: Tuple[Dict[str, torch.Tensor]],
    #     detected_instances: Optional[List[Instances]] = None,
    ...
    #         probabilities = np.array(values) / np.sum(values) if np.sum(values) != 0 else np.zeros_like(values)
    #         ent = entropy(probabilities, base=2)  # Base-2 entropy
    #         max_ent = np.log2(len(values)) if len(values) > 1 else 0  # Maximum entropy
    #         return ent / max_ent if max_ent != 0 else 0

    #     class_entropy_variation = {
    #         cls: 1 - calculate_entropy(values) for cls, values in class_values.items()
    #     }
        
    #     return ret, class_entropy_variation
