...
def post_process_instances(instances, cfg, device):
    if cfg.AGREEMENT.MODE == "concat":
        ...
        #     # 각 인스턴스의 마스크 면적 계산
        #     mask_areas = instance.pred_masks.float().sum((1, 2))
        #     # 각 인스턴스의 클래스 정보 가져오기
        #     classes = instance.pred_classes.cpu().numpy()
            
        #     # 클래스 별로 마스크 면적 추가
        #     for cls, area in zip(classes, mask_areas):
        #         areas_per_class[cls].append(area.item())
