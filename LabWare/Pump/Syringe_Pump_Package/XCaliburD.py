...
class XCaliburD(Syringe):
        ...
        Args:
            `com_link` (Object) : instantiated TecanAPI subclass / transport
                ...
                *Must have a `.sendRcv(cmd)` instance method to send a command
                    string and parse the response (see transport.py)
        Kwargs:
    ...
    def changePort(self, to_port, from_port=None, direction='CW'):
        ...
        diff = -delta if abs(delta) >= 7 else deltaDIR_DICT
...
if __name__ == '__main__':
    syringe_pump = TecanAPISerial(tecan_addr=2, ser_port="/dev/ttyUSB2", ser_baud=9600, ser_timeout=0.1, max_attempts=5)
    syringe_obj = XCaliburD(com_link=syringe_pump, num_ports=3, syringe_ul=5000)
    syringe_obj.init()
