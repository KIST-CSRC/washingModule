...
class NextPumpParemter():
    ...
    def preprocessPumpInformation(self):
        ...
        ex) {
            ...
            'solvent_type': ['Remove', 'Ethanol', 'H2O']
        }
        """
...
if __name__ == "__main__":
    ...
    operation_params = {
        "RemoveLiquid": {"target_flow": 600, "target_time": 10},
        "Ethanol": {"target_flow": 600, "target_time": 20},
        # "H2O": {"target_flow": 800, "target_time": 15},
    }    
    # __init__(self, logger_obj, device_name = "Next3000FJ",Nextpump_Addr=0,solution_name="",PORT="/dev/ttyPump2",set_direction=0):
    print(param_info_dict)
    for solvent_type, pump_info in param_info_dict.items():
            # Next3000FJ 객체 생성
            pump_obj = Next3000FJ(logger_obj=NodeLogger_obj, 
            ...
            # 각 펌프에 대해 다른 작동 매개변수를 사용하여 operate 메서드 호출
            if solvent_type in operation_params:
                params = operation_params[solvent_type]
                pump_obj.operate(target_flow=params["target_flow"], target_time=params["target_time"], mode_type="real")