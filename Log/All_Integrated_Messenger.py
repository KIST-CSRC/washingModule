...
# -*- coding: utf-8 -*-
# ##
# @brief    [IntegratedMessenger] IntegratedMessenger Class to send our error message to researcher to prevent error immediately
...
import os, sys
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "Log/MessageAPI")))
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "MessageAPI")))  # get import path : DSR_ROBOT.py
sys.path.append(
...
from mail_Class import MailMessenger
from telegram_Class import TelegramMessenger
from kakaotalk_Class import KakaotalkMessenger
from line_Class import LineMessenger

dt_now = datetime.datetime.now()
d_today = datetime.date.today()


class IntegratedMessenger:
    """
    [IntegratedMessenger] IntegratedMessenger Class to send our error message to researcher to prevent error immediately
    ...
    """ 
    def __init__(self, logger_obj, information_path="Log", mode_type="virtual"):
        """
    ...
    def MAIL_alert(self, toPerson_ID_list, text, *image_information):
        ...
        :return: None
        """
        self.__mail_messenger = MailMessenger(logger_obj=self.logger_obj, MAIL_GMAIL_USER_ID=self.__MAIL_GMAIL_USER["MAIL_GMAIL_USER_ID"], MAIL_GMAIL_USER_PWD=self.__MAIL_GMAIL_USER["MAIL_GMAIL_USER_PWD"], mode_type=self.__mode_type)
    ...
    def TELEGRAM_alert(self, text, *image):
        ...
        for user in self.__TELEGRAM_USER:
            ...
            self.__telegram_messenger = TelegramMessenger(logger_obj=self.logger_obj, TELEGRAM_CHAT_ID=TELEGRAM_CHAT_ID, TELEGRAM_TOKEN=TELEGRAM_TOKEN, mode_type=self.__mode_type)
            if image == True:
    ...
    def LINE_alert(self, text, *image):
        ...
        for user in self.__LINE_USER:
            ...
            self.__line_messenger = LineMessenger(logger_obj=self.logger_obj, LINE_TARGET_URL=LINE_TARGET_URL, LINE_TOKEN=LINE_TOKEN, mode_type=self.__mode_type)
            if image == True:
...
def AlertError(logger_obj, text_content, key_path, image_path, message_platform_list, mode_type='virtual'):
    """
    send Message to Researcher to notify our system's errors

    :param logger_obj (object): Logger object
    ...
    :return: True
    """
    messenger_bot = IntegratedMessenger(logger_obj, key_path=key_path, mode_type=mode_type)
...
def AlertMessage(logger_obj, text_content, key_path, message_platform_list, mode_type="virtual"):
    """
    ...
    # messenger_bot.TELEGRAM_alert(text=text_content, image=image_byte_content_read)
    # time.sleep(3)
    messenger_bot = IntegratedMessenger(logger_obj=logger_obj,information_path=key_path, mode_type=mode_type)
