...
TORQUE_DISABLE              = 0                 # Value for disabling the torque         # and this value (note that the Dynamixel would not move when the position value is out of movable range. Check e-manual about the range of the Dynamixel you use.)
...
def velocity_angular (DXL_ID, dxl_profile_velocity, dxl_angular):
    ...
    dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, DXL_ID, ADDR_PRO_PROFILE_VELOCITY,
    ...
    dxl_goal_position = int(dxl_angular * (4095/360))
...
velocity_angular (8, 50, 270)
velocity_angular (6, 20, 150)
