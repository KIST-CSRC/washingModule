...
def rotation_angular(DXL_ID, dxl_profile_velocity, dxl_angular):
    ...
    dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, DXL_ID, ADDR_PRO_PROFILE_VELOCITY, dxl_profile_velocity)
    if dxl_comm_result != COMM_SUCCESS:
        print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
    elif dxl_error != 0:
        print("%s" % packetHandler.getRxPacketError(dxl_error))

    dxl_displacement = int(dxl_angular * (4095/360))

    # Write goal position
    dxl_present_position, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, DXL_ID, ADDR_PRESENT_POSITION)
    dxl_goal_position = dxl_present_position + dxl_displacement
