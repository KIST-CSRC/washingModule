...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "MOTOR_TEST")))  # get import path : DSR_ROBOT.py
...
from MOTOR_TEST.DYNAMIXEL.src.dynamixel_sdk import *
...
groupSyncWrite1 = GroupSyncWrite(portHandler, packetHandler, ADDR_PRO_PROFILE_VELOCITY, LEN_PRO_PROFILE_VELOCITY)
groupSyncWrite2 = GroupSyncWrite(portHandler, packetHandler, ADDR_PRO_GOAL_POSITION, LEN_PRO_GOAL_POSITION)
...
def syncRotation(DXL1_ID, dxl1_profile_velocity, dxl1_goal_position, DXL2_ID, dxl2_profile_velocity, dxl2_goal_position):
    ...
    param_profile_velocity = [DXL_LOBYTE(DXL_LOWORD(dxl1_profile_velocity)),
                                  DXL_HIBYTE(DXL_LOWORD(dxl1_profile_velocity)),
                                  DXL_LOBYTE(DXL_HIWORD(dxl1_profile_velocity)),
                                  DXL_HIBYTE(DXL_HIWORD(dxl1_profile_velocity))]

    # Add Dynamixel#1 profile velocity value to the Syncwrite parameter storage
    dxl_addparam_result = groupSyncWrite1.addParam(DXL1_ID, param_profile_velocity)
    if dxl_addparam_result != True:
        print("[ID:%03d] groupSyncWrite addparam failed" % DXL1_ID)
        quit()

    # Allocate profile velocity value into byte array
    param_profile_velocity = [DXL_LOBYTE(DXL_LOWORD(dxl2_profile_velocity)),
                                 DXL_HIBYTE(DXL_LOWORD(dxl2_profile_velocity)),
                                 DXL_LOBYTE(DXL_HIWORD(dxl2_profile_velocity)),
                                 DXL_HIBYTE(DXL_HIWORD(dxl2_profile_velocity))]

    # Add Dynamixel#2 profile velocity value to the Syncwrite parameter storage
    dxl_addparam_result = groupSyncWrite1.addParam(DXL2_ID, param_profile_velocity)
    if dxl_addparam_result != True:
    ...
    param_goal_position = [DXL_LOBYTE(DXL_LOWORD(dxl1_goal_position)),
    ...
    dxl_addparam_result = groupSyncWrite2.addParam(DXL1_ID, param_goal_position)
    if dxl_addparam_result != True:
        print("[ID:%03d] groupSyncWrite addparam failed" % DXL1_ID)
        quit()

    # Allocate goal position value into byte array
    param_goal_position = [DXL_LOBYTE(DXL_LOWORD(dxl2_goal_position)),
    ...
    dxl_addparam_result = groupSyncWrite2.addParam(DXL2_ID, param_goal_position)
    if dxl_addparam_result != True:
        print("[ID:%03d] groupSyncWrite addparam failed" % DXL2_ID)
        quit()
    ...
    while 1:
        ...
        DXL1_ID, dxl1_goal_position, dxl1_present_position, DXL2_ID, dxl2_goal_position,
