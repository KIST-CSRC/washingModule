    ...
    try:
        for epoch in range(num_epochs):
            ...
            if (epoch+1)*epoch_size < iteration:
                continue
            for batch_idx, datum in enumerate(data_loader):
                try:
                    print(f"Processing batch {batch_idx+1}/{len(data_loader)} of epoch {epoch+1}")
                    
                    # 여기에 기존 배치 처리 코드를 추가합니다
                    losses = net(datum)
                    ...

                    print(f"Finished batch {batch_idx+1}/{len(data_loader)} of epoch {epoch+1}")
                except Exception as e:
                    print(f"Error during processing batch {batch_idx+1} of epoch {epoch+1}")
                    print(f"Error: {e}")
                    # 필요한 경우 여기에 오류의 자세한 정보를 추가하여 디버깅 정보를 얻을 수 있습니다.
                    # 예를 들어, datum의 내용을 출력하거나 오류의 스택 트레이스를 출력할 수 있습니다.
                    break  # 혹은 raise e를 사용하여 오류를 다시 발생시킬 수 있습니다.
